...
def _rag_preview_logic(q: str, auto_rag: Optional[bool] = None) -> Dict[str, Any]:
    ...
    use_rag = auto_rag if auto_rag is not None else _get_current_rag_state()
    if not use_rag:
...
def get_config() -> Dict[str, Any]:
    ...
    cfg.update({
        ...
        "version": 1,
        # Ensure RAG settings are read from environment
        "auto_rag": _get_current_rag_state(),
        "rag_top_k": int(os.environ.get("RAG_TOP_K", "4")),
        "rag_max_chars": int(os.environ.get("RAG_MAX_CHARS", "1800")),
        # Ensure guardian settings are read from environment
        "guardian_enabled": os.environ.get("GUARDIAN_ENABLED", "true").lower() in ("true", "1", "yes"),
    })
    return cfg
...
def toggle_rag(request: Request) -> Dict[str, Any]:
    ...
    # Get current RAG state from environment
    current_rag = os.environ.get("AUTO_RAG", "false").lower() in ("true", "1", "yes")
    
    # Toggle to opposite state
    new_rag_state = not current_rag
    
    # Update environment variable
    os.environ["AUTO_RAG"] = str(new_rag_state).lower()
    
    # Update config to match
    cs.config["auto_rag"] = new_rag_state
    cs.save_config()
    
    # Update .env file
    _update_env_file("AUTO_RAG", str(new_rag_state).lower())
    
    # Reload environment to ensure changes take effect
    _reload_environment()
    
    return {
        "auto_rag": new_rag_state,
        "message": f"RAG {'enabled' if new_rag_state else 'disabled'}",
...
def set_rag_state(body: Dict[str, Any], request: Request) -> Dict[str, Any]:
    ...
    if "auto_rag" not in body:
        raise HTTPException(status_code=400, detail={"error": {"code": "MISSING_PARAMETER", "detail": "auto_rag parameter is required"}})
    
    new_rag_state = bool(body["auto_rag"])
...
def toggle_guardian(request: Request) -> Dict[str, Any]:
    ...
    # Get current Guardian state from environment
    current_guardian = os.environ.get("GUARDIAN_ENABLED", "true").lower() in ("true", "1", "yes")
    
    # Toggle to opposite state
    new_guardian_state = not current_guardian
    
    # Update environment variable
    os.environ["GUARDIAN_ENABLED"] = str(new_guardian_state).lower()
    
    # Update config to match
    cs.config["guardian_enabled"] = new_guardian_state
    cs.save_config()
    
    # Update .env file
    _update_env_file("GUARDIAN_ENABLED", str(new_guardian_state).lower())
    
    # Reload environment to ensure changes take effect
    _reload_environment()
    
    return {
        "guardian_enabled": new_guardian_state,
        "message": f"Guardian {'enabled' if new_guardian_state else 'disabled'}",
        "environment_updated": True
    }
...
def set_guardian_state(body: Dict[str, Any], request: Request) -> Dict[str, Any]:
    ...
    if "guardian_enabled" not in body:
        raise HTTPException(status_code=400, detail={"error": {"code": "MISSING_PARAMETER", "detail": "guardian_enabled parameter is required"}})
    
    new_guardian_state = bool(body["guardian_enabled"])
...
def clear_knowledge_base(request: Request) -> Dict[str, Any]:
    ...
    _require_auth(request)
    try:
        kb_path = os.path.join(os.path.dirname(__file__), "chat_core", "kb")
        
        # Clear meta.json
        meta_file = os.path.join(kb_path, "meta.json")
        if os.path.exists(meta_file):
            with open(meta_file, 'w') as f:
                f.write('[]')
        
        # Clear FAISS index
        faiss_file = os.path.join(kb_path, "faiss.index")
        if os.path.exists(faiss_file):
            os.remove(faiss_file)
        
        # Clear titles.npy
        titles_file = os.path.join(kb_path, "titles.npy")
        if os.path.exists(titles_file):
            os.remove(titles_file)
        
        return {
            "message": "Knowledge Base cleared successfully",
            "files_cleared": ["meta.json", "faiss.index", "titles.npy"]
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail={"error": {"code": "KB_CLEAR_ERROR", "detail": str(e)}})
...
def clear_chat_history(request: Request) -> Dict[str, Any]:
    ...
    try:
        ...
        history_files = [
            os.path.join(os.path.dirname(__file__), "chat_core", "history_last.jsonl"),
            os.path.join(os.path.dirname(__file__), "..", "chat_core", "history_last.jsonl")
        ]
        ...
        for history_file in history_files:
            if os.path.exists(history_file):
                with open(history_file, 'w') as f:
                    f.write('')
                cleared_files.append(os.path.basename(history_file))
        ...
        raise HTTPException(status_code=500, detail={"error": {"code": "CHAT_CLEAR_ERROR", "detail": str(e)}})
...
def _get_current_rag_state() -> bool:
    """Get current RAG state from environment"""
    return os.environ.get("AUTO_RAG", "false").lower() in ("true", "1", "yes")
...
async def chat_stream(body: StreamChatBody, request: Request) -> Response:
    ...
    use_rag = _get_current_rag_state() if body.auto_rag is None else bool(body.auto_rag)
